...
describe("executeAction: interaction", () => {
	...
	it("reports TARGET_NOT_FOUND, not a crash, when no element matches", async () => {
		...
		expect(result).toEqual({ outcome: "failed", error: { code: "TARGET_NOT_FOUND", message: expect.any(String) } });
	});
	...
	it("reports TARGET_AMBIGUOUS, never a silent first-match pick", async () => {
		...
		expect(result).toEqual({ outcome: "failed", error: { code: "TARGET_AMBIGUOUS", message: expect.any(String) } });
	});
	...
	it("reports ACTION_FAILED, not a crash, when a resolved element is never actionable", async () => {
		...
		try {
			...
			expect(result).toEqual({ outcome: "failed", error: { code: "ACTION_FAILED", message: expect.any(String) } });
		}
		finally {
...
describe("executeAction: read/assign", () => {
	it("read text copies innerText into the destination output", async () => {
		...
		expect(result).toEqual({ outcome: "succeeded", extraction: { fieldName: "balance", value: "1234.50" } });
	...
	it("read value copies the raw input value into the destination output", async () => {
		...
		expect(result).toEqual({ outcome: "succeeded", extraction: { fieldName: "accountNumber", value: "ACCT-1042" } });
	});
	...
	it("read number parses the control value into a number", async () => {
		...
		expect(result).toEqual({ outcome: "succeeded", extraction: { fieldName: "balance", value: 1234.5 } });
	});
	...
	it("read number on an unparsable string fails with INVALID_NUMBER", async () => {
		...
		expect(result).toEqual({ outcome: "failed", error: { code: "INVALID_NUMBER", message: expect.any(String) } });
	});
	...
	it("read number on an empty string assigns null", async () => {
		...
		expect(result).toEqual({ outcome: "succeeded", extraction: { fieldName: "notes", value: null } });
	});
	...
	it("read value at a point resolves the underlying HTML value-bearing control", async () => {
		...
		expect(result).toEqual({ outcome: "succeeded", extraction: { fieldName: "deep", value: "deep-value" } });
	});
	...
	it("read text at a point falls back to the element's displayed text", async () => {
		...
		expect(result).toEqual({ outcome: "succeeded", extraction: { fieldName: "belowFold", value: "Below the fold" } });
	});
	...
	it("assign resolves a scalar value and records it as an output, including null", async () => {
		...
		expect(result).toEqual({ outcome: "succeeded", extraction: { fieldName: "status", value: "found" } });

		const nullResult = await executeAction(fixture.page, { id: "s", action: "assign", args: [{ ref: "output", name: "extra" }, null] }, ctx);
		expect(nullResult).toEqual({ outcome: "succeeded", extraction: { fieldName: "extra", value: null } });
	});
});
...
describe("executeAction: check/verify/goto/finish/fail", () => {
	it("check reports the boolean answer without failing when false", async () => {
		...
		expect(result).toEqual({ outcome: "succeeded", checked: false });
	});
	...
	it("verify fails with VERIFY_TIMEOUT when the condition never becomes true", async () => {
		...
		expect(result).toEqual({ outcome: "failed", error: { code: "VERIFY_TIMEOUT", message: expect.any(String) } });
	});
	...
	it("goto reports the jump target without touching the page", async () => {
		...
		expect(result).toEqual({ outcome: "succeeded", gotoStepId: "complete" });
	});
	...
	it("finish with a null checkpoint always succeeds finished", async () => {
		...
		expect(result).toEqual({ outcome: "succeeded", finished: true });
	});
	...
	it("finish evaluates its checkpoint condition", async () => {
		...
		const result = await executeAction(fixture.page, step, newContext());
		expect(result).toEqual({ outcome: "succeeded", finished: false });
	});
	...
	it("fail reports a businessFailure outcome with its code/message", async () => {
		...
		expect(result).toEqual({ outcome: "failed", kind: "businessFailure", error: { code: "UNKNOWN_RESULT", message: "No recognized search outcome" } });
	});
});
