...
import { type ActionOutcome, executeAction } from "./actions.ts";
...
import { createOutputsState } from "./outputsState.ts";
import type { ChildStep, RecipeDefinition, Recovery, ScalarValue, Step } from "./types.ts";
...
async function maskedScreenshot(page: Page, secrets: string[]): Promise<Buffer> {
	if (secrets.length === 0) {
...
async function captureAndUploadArtifact(deps: LoopDeps, stepId: string): Promise<void> {
	try {
		const screenshot = await maskedScreenshot(deps.page, deps.secrets);
