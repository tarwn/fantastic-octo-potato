...
function baseContext(overrides: Partial<RecipeCompilationContext> = {}): RecipeCompilationContext {
	return {
		...
		executedSteps: [
			{ id: "open_starting_url", intent: "do open_starting_url", action: "open", args: [{ ref: "input", name: "startingUrl" }] },
			{
				id: "read_total",
				intent: "do read_total",
				action: "read",
				args: [{ by: "text", value: "Total" }, "text", { ref: "output", name: "total" }]
			},
			{ id: "s3", intent: "do s3", action: "finish", args: [null] }
		],
...
describe("compileRecipe", () => {
	it("compiles a valid response into a RecipeDefinition that carries the executed Steps over verbatim", async () => {
		...
		expect(result).toEqual({
			...
			steps: [
				...
				{ id: "s3", intent: "do s3", action: "finish", args: [{ test: "all", args: [{ test: "assigned", args: [{ ref: "output", name: "total" }] }] }] }
			],
			recoveries: []
		});
	});
	...
	it("builds a trivially-satisfied checkpoint when the run produced no outputs", async () => {
		...
		const result = await compileRecipe(
			baseContext({
				executedSteps: [{ id: "s1", intent: "do s1", action: "finish", args: [null] }],
				results: []
			})
		);

		expect(result.steps).toEqual([{ id: "s1", intent: "do s1", action: "finish", args: [{ test: "all", args: [] }] }]);
	});
	...
	it("accepts an executed Step referencing a credential name the Training run reported", async () => {
		...
		const result = await compileRecipe(
			baseContext({
				executedSteps: [
					{ id: "open_starting_url", intent: "do open_starting_url", action: "open", args: [{ ref: "input", name: "startingUrl" }] },
					{ id: "fill_user", intent: "do fill_user", action: "fill", args: [{ by: "label", value: "Username" }, { ref: "credential", name: "loginUser" }] },
					{
						id: "read_total",
						intent: "do read_total",
						action: "read",
	...
	it("fails loudly when an executed Step references a credential name the Training run never reported", async () => {
		...
		await expect(
			compileRecipe(
				baseContext({
					executedSteps: [
						{ id: "fill_user", intent: "do fill_user", action: "fill", args: [{ by: "label", value: "Username" }, { ref: "credential", name: "guessed" }] },
						{ id: "s3", intent: "do s3", action: "finish", args: [null] }
					],
					credentialNames: ["loginUser"]
				})
	...
	it("fails loudly when an executed Step has no intent", async () => {
		mockedSendChatCompletion.mockResolvedValue(JSON.stringify(VALID_RESPONSE));
		const { compileRecipe, RecipeCompilationInvalidResponseError } = await import("./recipeCompilation");
		...
		await expect(
			compileRecipe(
				baseContext({
					executedSteps: [
						{ id: "click_go", action: "click", args: [{ by: "text", value: "Go" }] },
						{ id: "s3", intent: "do s3", action: "finish", args: [null] }
					]
				})
			)
