...
describe("runnerActions", () => {
	...
	describe("reportJobStep", () => {
		it("rejects a missing/incorrect bearer secret", () => {
			...
			const result = reportJobStep(db, String(runnerId), String(jobId), "Bearer wrong-secret", SHARED_SECRET, {
				sequence: 1,
				kind: "step",
				text: "did a thing"
			});
		...
		it("rejects an unknown runner id", () => {
			...
			const result = reportJobStep(db, "999", String(jobId), `Bearer ${SHARED_SECRET}`, SHARED_SECRET, {
				sequence: 1,
				kind: "step",
				text: "did a thing"
			});
		...
		it("rejects an unknown job id", () => {
			...
			const result = reportJobStep(db, String(runnerId), "999", `Bearer ${SHARED_SECRET}`, SHARED_SECRET, {
				sequence: 1,
				kind: "step",
				text: "did a thing"
			});
		...
		it("rejects a Runner not assigned to the Job with 403", () => {
			...
			const result = reportJobStep(db, String(otherRunnerId), String(jobId), `Bearer ${SHARED_SECRET}`, SHARED_SECRET, {
				sequence: 1,
				kind: "step",
				text: "did a thing"
			});
		...
		it("rejects a missing text with 400", () => {
			const db = getDb();
			const runnerId = seedRunner(db);
			const jobId = insertPendingJob(db, 1);
			runnerPoll(db, String(runnerId), `Bearer ${SHARED_SECRET}`, SHARED_SECRET);
