# ADR 0002: Husky for git hooks

**Author:** Agent (Eli Weinstock-Herman)

The repo needs pre-commit and pre-push enforcement: pre-commit runs `guard` across all 3 scopes, pre-push runs `tools/hooks/check-review.cjs` (the code-review staleness/critical-findings gate). The setup todo directs using husky by name for both hooks.
...
- `guard` must stay fast enough to run on every commit across all 3 scopes, or contributors will feel friction on every commit
