...
* Any write that is only valid in a given Job status (or for a given owner) must check that status/owner in the same transaction as the write, and anything still pending must be voided when the Job leaves that status — never rely on an earlier read. This is how a late operator command or result cannot revive a Job that was cancelled or ended ([ADR 0005](../../adrs/hub/0005-operator-command-wire-protocol.md)).
