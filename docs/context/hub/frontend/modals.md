...
Pattern for a modal dialog local to a page's `_components` folder — copy [StartTrainingModal.svelte](../../../../src/hub/src/routes/registered-applications/[id]/_components/StartTrainingModal.svelte) as the reference implementation; keep future modals consistent with it rather than inventing a new approach per modal.
