...
Also see: [Runner API: init, poll, and Job steps](../hub/backend/runner-api.md) (the hub side of this contract), [Runner HTTP Handling](./runner-http-handling.md) (status-code reactions).
...
* Job-aware main loop: a poll response with `hasWork: true` pauses the poll interval (`jobLoopRunning` flag) and hands the claimed Job to `runJobLoop` (`pollLoop.ts`), which follows the `nextStep` chain via `reportStep` until a response carries no `nextStep` — never counting steps or deciding an outcome itself (Hub is authoritative, see [Job Queue](../hub/job-queue.md)) — then resumes polling.
