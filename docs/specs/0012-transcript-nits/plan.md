...
- [x] 7. Bug: when `executeAction` throws an unexpected error, the Step is likewise not reported before the Completed-Error status (Recipe and Training loops). Needs a decision on what `targetDescription` to send, since none is available.
