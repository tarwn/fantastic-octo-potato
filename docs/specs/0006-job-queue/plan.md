...
- Job and transcript structural change: [spec-job-and-transcript-change.md](./spec-job-and-transcript-change.md)
    - [ ] DB schema: lookup tables, Job base/extension split, safe/raw ingredient and result columns
    - [ ] Repository layer: base+extension Job reads, safe/raw ingredient and result reads, structured transcript serialization
    - [ ] Hub action/API layer: kind-dispatched `steps` endpoint, ingredient/result sensitivity resolution
